...
def run_pipeline(regenerate=False, provider=None, judge_provider=None, on_stage=None):
    """Runs the full recording -> transcript -> summary pipeline, writing into
    OUTPUT_DIR. on_stage(stage), if given, fires before each step — stages are
    'recording', 'transcribing', 'summarizing', 'judging' (only when
    judge_provider is given), 'done' — so a caller (the CLI, or the webapp)
    can report progress. judge_provider is None by default (skip judging) so
    plain CLI runs stay exactly as fast/cheap as before; pass 'local' or
    'claude' to score the result and append it to run_history.jsonl."""
