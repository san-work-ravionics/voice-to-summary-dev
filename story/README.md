# Story — 5 meetings, baseline vs. context-aware summarization over time

Part of the [voice-to-summary](../README.md) project. v2 already showed that giving the summarizer *static* project background produces a sharper single-meeting summary. This asks a harder question: across a **series** of meetings, does giving the summarizer a **running history of what happened in prior meetings** produce a better summary than treating each meeting in isolation?

## The story

Five weekly Monday status syncs about the same Mobile App Redesign project (same premise as v1-v4), in the weeks leading up to a stakeholder review. Unlike v1-v4's single snapshot, this is a real arc — each week's dialogue references earlier weeks by name:
...
Budget/cost impact is never discussed on any of the 5 calls — called out explicitly in week 5, echoing the same deliberate checklist gap from [v3](../v3/README.md)/[v4](../v4/README.md).
...
- **`summarize_with_context`** — the project's static background (like v2) **plus a running history** built from the Decisions + Actions of every previous meeting's own context-aware summary. Meeting 4's context literally depends on what meeting 1-3 actually produced, not a hand-written script — the same way a person catching up from real meeting notes would.
