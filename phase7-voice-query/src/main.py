...
def run_query(audio_path, provider=None, top_k=5, on_stage=None):
    ...
    answers grounded in those chunks. Returns a dict with the answer,
    retrieved sources, per-stage timing, and a deterministic grounding
    score; also writes all of it to output/query-N/."""
    ...
    retrieved = query_index(index, question_text, top_k=top_k)
    timings["retrieve_s"] = time.time() - t0

    on_stage("answering")
    t0 = time.time()
    answer_text = answer_question(question_text, retrieved, provider=provider)
...
def main():
    parser = argparse.ArgumentParser(description="Phase 6 — ask a spoken question, get a grounded text answer")
    ...
    result = run_query(args.audio_path, provider=args.provider, top_k=args.top_k, on_stage=on_stage)
