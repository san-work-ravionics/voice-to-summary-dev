...
def _pick_voices():
    # Picked once and reused across all 15 meetings so the same two people
    # sound the same from kickoff through launch, like a real recurring pair.
...
def generate_all(output_dir_template="output/{slug}/recording.wav"):
    narrator_id, voice_a_id, voice_b_id = _pick_voices()
    paths = []
    for meeting in MEETINGS:
        output_path = output_dir_template.format(slug=meeting["slug"])
